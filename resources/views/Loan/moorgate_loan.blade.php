...
@endsection
{{-- END PAGE LEVEL CSS --}}
@section('title','Moorgate Loan Setup')
{{-- BEGIN CONTENT --}}
@section('content')
<!-- Begin Page Content -->
<div class="container-fluid">
   ...
   <div class="card">
      <div class="card-body">
         <form action="{{ route('moorgate_loan') }}" method="get">
            <div class="row">
   ...
   <div class="card">
      <div class="card-body">
         <div class="row">
            <div class="col-6">
               <h4 class="card-title">Moorgate Loan Setup</h4>
            </div>
                  ...
                  @if(count($loan) > 0)
                     ...
                     @foreach($loan as $row)
                        <tr>
                           ...
                           <td class="text-center">
                              <div class="w-100">
                                 <a href="javascript:(0)" class="text-dark update_moorgate_loan" data-amount_deduction="{{ $row->amount_deduction }}" data-remarks="{{ $row->remarks }}" data-date_to="{{ $row->date_to }}" data-date_from="{{ $row->date_from }}" data-loan_id="{{ $row->loan_id }}" data-emp_name="{{ $row->emp_name }}" data-loan_date="{{ $row->loan_date }}" data-loan_amount="{{ $row->loan_amount }}">
                           ...
                           <td class="text-center">
                              <div class="w-100">
                                 <a href="{{ url('/delete_moorgate_loan?id=').$row->loan_id }}" onclick="return confirm('Delete Loan')" class="text-danger">
   ...
   <div class="modal-dialog modal-lg">
      <div class="modal-content">
         <div class="modal-header bg-primary text-white">
            <h4 class="modal-title">New Moorgate Loan</h4>
            <button type="button" class="close text-danger" data-dismiss="modal" aria-hidden="true" title="Close">×</button>
         </div>
         <form class="form-material" action="{{ url('/add_moorgate_loan') }}" method="post">
            @csrf
...
{{-- UPDATE MODAL --}}
<div id="update_moorgate_loan" class="modal fade show" tabindex="-1" role="dialog" aria-labelledby="myModalLabelUpdate" style=" padding-right: 17px;">
   <div class="modal-dialog modal-lg">
      <div class="modal-content">
         <div class="modal-header bg-success text-white">
            <h4 class="modal-title">Update Moorgate Loan</h4>
            <button type="button" class="close text-danger" data-dismiss="modal" aria-hidden="true" title="Close">×</button>
         </div>
         <form class="form-material" action="{{ url('/update_moorgate_loan') }}" method="post">
            @csrf
...
<script>
   $(document).ready(function() {
      ...
      $(document).on('click', '.update_moorgate_loan', function(e) {
