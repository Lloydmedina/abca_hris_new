                  ...
                  @foreach($employees as $row)
                     <tr>
                        {{-- <td class="text-center">
                           <a href="{{ asset($row->picture_path) }}" target="_blank">
                              <img class="rounded-circle" style="width: 30px; height: 30px; cursor: pointer;" src="{{ asset($row->picture_path) }}" alt="Profile photo">
                           </a>
                        </td> --}}
                        <td>{{ $row->UserID_Empl }}</td>
                        <td>
                           <a href="{{ url('employee?id='.$row->SysPK_Empl.md5( $row->SysPK_Empl) ) }}" class="text-info" title="View Information">
                                {{ strtoupper($row->Name_Empl) }}
                              {{-- {{ ucwords(strtolower($row->Name_Empl)) }} --}}
                           </a>
                        </td>
                        <td>{{ $row->contact_no }}</td>
                        {{-- <td>{{ str_limit($row->Address_Empl, 30) }}</td> --}}
                        <td>{{ $row->company }}</td>
                        <td>{{ $row->outlet }}</td>
                        ...
                        <td class="text-center">
                        <a href="{{ url('update-employee?id='.$row->SysPK_Empl.md5( $row->SysPK_Empl) ) }}" class="text-success">
